...
export function authenticate(req){
  try{
    ...
    return {login:cookieSession.login,name:cookieSession.name,role:cookieSession.role,actorInitials:cookieSession.actorInitials||'',session:'cookie'};
  }catch{return null;}
}
...
export function actorName(_req,user){const initials=user?.actorInitials||'';return initials?`${user.name||user.login} · ${initials}`:(user.name||user.login);}
