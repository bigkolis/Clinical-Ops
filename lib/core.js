...
export function credentials(login,password){const l=String(login||'').trim(),p=String(password||'');if(process.env.AUTH_USE_ENV==='1'){const envUsers=[{login:process.env.MILANA_LOGIN,password:process.env.MILANA_PASSWORD,name:process.env.MILANA_DISPLAY_NAME||'Milana',role:'milana'},{login:process.env.STAFF_LOGIN,password:process.env.STAFF_PASSWORD,name:process.env.STAFF_DISPLAY_NAME||'Clinical Ops Staff',role:'staff'}].filter(x=>x.login&&x.password);const hit=envUsers.find(x=>safeEq(l,x.login)&&safeEq(p,x.password));return hit?{login:hit.login,name:hit.name,role:hit.role}:null;}const hit=BUILTIN_USERS.find(x=>safeEq(l,x.login)&&verifyPassword(p,x));return hit?{login:hit.login,name:hit.name,role:hit.role}:null;}
export function authenticate(req){try{const header=String(req.headers?.authorization||'');if(!header.startsWith('Basic '))return null;const raw=Buffer.from(header.slice(6),'base64').toString('utf8'),i=raw.indexOf(':');if(i<1)return null;return credentials(raw.slice(0,i),raw.slice(i+1));}catch{return null;}}
export function actorName(req,user){const initials=String(req.headers?.['x-actor-initials']||'').trim().replace(/[^A-Za-zА-Яа-я0-9._-]/g,'').slice(0,16);return initials?`${user.name||user.login} · ${initials}`:(user.name||user.login);}
export function dbEnabled(){return Boolean(process.env.DATABASE_URL);}
export function db(){if(!dbEnabled())return null;if(!dbClient)dbClient=postgres(process.env.DATABASE_URL,{ssl:'require',max:2,idle_timeout:10,connect_timeout:10});return dbClient;}
export async function ensureSchema(){if(!dbEnabled())return false;if(!schemaPromise){const sql=db();schemaPromise=(async()=>{
